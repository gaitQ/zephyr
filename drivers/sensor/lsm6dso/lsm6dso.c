				...
				const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LPF_FREQ:
		...
		// Configure Accel LPF2 cutoff frequency
			       ...
			       const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LPF_FREQ:
		...
		// Configure Gyro LPF1 cutoff frequency
...
static int lsm6dso_sample_fetch_fifo(const struct device *dev)
{
	...
	uint16_t data_fifo;
	lsm6dso_fifo_data_level_get(ctx, &data_fifo);
	lsm6dso_fifo_tag_t data_tag;
	for(int i=0; i<data_fifo; i++) {
...
static int lsm6dso_init_chip(const struct device *dev)
{
	...
	if (lsm6dso_batch_counter_threshold_set(ctx, cfg->batch_cnt_thr)) {
		LOG_ERR("Setting FIFO batch count threshold Failed");
		return -EIO;
	}

	if (lsm6dso_fifo_xl_batch_set(ctx, cfg->accel_bdr)) {
		LOG_ERR("Enable Accel FIFO batch mode Failed");
		return -EIO;
	}

	if (lsm6dso_fifo_gy_batch_set(ctx, cfg->gyro_bdr)) {
		LOG_ERR("Enable Gyro FIFO batch mode Failed");
		return -EIO;
	}
#else
