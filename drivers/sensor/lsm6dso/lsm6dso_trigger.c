...
/**
 * lsm6dso_enable_fifo_bdr_int - BDR count THRESHOLD enable selected int pin to generate interrupt
 */
static int lsm6dso_enable_fifo_bdr_int(const struct device *dev, int enable)
{
	const struct lsm6dso_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
			  ...
			  sensor_trigger_handler_t handler)
{
	...
	if (trig->chan == SENSOR_CHAN_ACCEL_XYZ) {
		if (trig->type == SENSOR_TRIG_DATA_READY) {
			...
			lsm6dso->handler_fifo_bdr = handler;
			if (handler) {
				if (lsm6dso_fifo_cnt_event_batch_set(ctx, LSM6DSO_XL_BATCH_EVENT)) {
					return -EIO;
				}
				return lsm6dso_enable_fifo_bdr_int(dev, LSM6DSO_EN_BIT);
			} else {
				return lsm6dso_enable_fifo_bdr_int(dev, LSM6DSO_DIS_BIT);
			}
		}
#endif
...
static void lsm6dso_handle_interrupt(const struct device *dev)
{
	...
	while (1) {
		...
		if (lsm6dso_fifo_status_get(ctx, &fifo_status2) < 0) {
			LOG_ERR("failed reading fifo status2 reg");
			return;
		}
#endif
...
#endif
#if defined(CONFIG_LSM6DSO_ENABLE_FIFO)
					&& (lsm6dso->handler_fifo_bdr == NULL || fifo_status2.counter_bdr_ia == 0)
#endif
		) {
			break;
		}
...
#if defined(CONFIG_LSM6DSO_ENABLE_FIFO)
		if ((fifo_status2.counter_bdr_ia) && (lsm6dso->handler_fifo_bdr != NULL)) {
			lsm6dso->handler_fifo_bdr(dev, &fifo_trigger);
		}
#endif
